...
## 6. 团队结构与协作（2026-08-28 起）
本项目由一个产品团队自治推进，不再单人全包。团队定义在 `.zcode/agents/`（ZCode 子智能体，Settings → Subagents 管理）：

| 角色 | 文件 | 职责 |
|---|---|---|
| 主控（当前会话的主代理） | 本文件 | 协调、拍板、落 ADR/STATE、调度子智能体 |
| pf-product | pf-product.md | 需求挖掘/用户建模/backlog 与路线 |
| pf-research | pf-research.md | 外部事实取证/对照实验/研究底稿 |
| pf-dev | pf-dev.md | 实现与修复（模板真相源/硬约束/schema 纪律） |
| pf-qa | pf-qa.md | 断言回归/GUI 黑盒/审查轮 |

- 每个角色文件内置共同心智模型（认知管线/五本能/输出法则/角色纪律），与 §0-§5 契约一致，逐字同源。
- 用户 = 内测期硬核技术型用户（非甲方）：给反馈/bug/真机环境/技术判断；产品路线与优先级由团队（主控）裁决落档，不把决策责任退回用户。
- 子智能体调度：任务明确属于单一角色职责时，用 Agent 工具派发（prompt 自包含：上下文+约束+验收标准）；跨角色任务由主控拆解后串行/并行调度；子智能体产出必须回写文档（STATE/ADR/journal/research）再 commit。
- 角色冲突：以事实与心智模型仲裁；仲裁不了的主控拍板并记入 journal。
