...
- 更新：2026-08-29 s47+s47b（P31-① 本机部分：v0.9.5 新包 → 沙盒全流程 → 双真任务冒烟全绿；抓出修复 3 问题——时区显示/搜索✕残留/goosehints 教坏 cmd 引号形态（取证 T1–T7c）；首个 git tag 定版 v0.9.3→0.9.5；openpyxl 失实声明修正。真机 POC 待用户；候选遗留见 journal s47）
