...
- 更新：2026-08-29 s47→s50 系列（s50 本会话累计 22 commits）：用户主线①数据引用统一+数据库可见性/可引用性全链路落地并 VERIFIED-RUN（报表卡@形态 /api/db/overview @菜单DB分组 管理面板清单 hints三条 降级态）；②性能安全审计两轮+修复 12 项（事件循环冻结//open/注入/POST上限/剪贴板detached/重发文案/welcome 404/WS协议面4缺陷/esc收敛/BOM/401误报/换线竞态）+P31-④裁决收窄（401自动切换永久裁掉，一键换线+错误分类落地，GUI全链路验证）；FIND-0 归因收口（pc 被子代理任务收割连带硬杀，dev-stack-up 改 detached 拉起，目标机形态天然免疫）；goose v1.48 上游情报入库。**v0.9.6 沙盒冒烟通过后叠加 7 fix → v0.9.7 打包中（护航候选）**。部署缺口：全新包无 API key 首任务 401（向导首屏应含配Key步）；s50e/f/g/h 修复随 v0.9.7 首次出海
