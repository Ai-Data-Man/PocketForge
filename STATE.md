...
- 更新：2026-08-29 s47/s47b/s48（P31-① 本机部分 ✅：v0.9.5 新包+沙盒全流程+双真任务冒烟+3 问题修复——时区显示/搜索✕残留/goosehints 教坏 cmd 引号形态（取证 T1–T7c）；P31-② 收窄版实现：报表引导卡（pf-pm 裁剪掉抄数据/查记忆卡）已进 dev 树，**GUI 点击流验证待下次会话复核**（IAB 输入管线中途失效，见 journal s48）；首个 git tag v0.9.3→0.9.5；openpyxl 失实声明修正。真机 POC 待用户）
