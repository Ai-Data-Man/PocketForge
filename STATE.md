...
- 更新：2026-09-05 s66（收尾）：**PG 阶段一落地**（ADR-0011 用户批准后当日完成设计+实施：zonky 17.11.0 进树 bin/pg 三 exe、pc 编排 pg-init 幂等 oneshot + pg daemon（readiness=pg_ctl status，max_restarts:3）、bootstrap Pick-Port base 5432 写 data/pg.port、两启动器导 PG_PORT；21 条验收全过 d252478；实测 initdb dev 12.6s/沙盒 18.3s、空闲内存全树约 76MB、包体压缩 +36.1MB、UTF8+C locale 线协议实读、127.0.0.1 独占监听；**顺藤挖出并修复既有产品缺陷：停止数字员工.cmd 用不存在的 pc shutdown 子命令（2>nul 吞错，栈从未真停）→down**；qa 通过+追加同族残留 update-runner.js:151 同死词（升级流程停栈从未生效，活栈解包锁险），已修 3da0502 探针实证）。**阶段二切片落地**（4a2d54d+fc948e9+3409fc1，qa 通过）：pg_dump.exe 进树（EDB zip 同源 0.59MB）、pg_dump 每日随 forge-backup（data/pg-dumps keep 3，PG 不在场 warn+zip 照常）、readiness 探活改 TCP（pg-probe.js 生成文件，内联 -e 被 cmd 三层引号链证伪）、postgres.js 3.4.9 客户端取证全过（零依赖/UTF8 往返/2s 超时精确命中，不入树随三件套）；**沙盒冷启实锤真缺陷**——PS 5.1 Compress-Archive -Update 追加空目录删除已生成 zip（全新机 daily-backup 必崩），fc948e9 一行修复+态 B0 自动化断言（e2e 升至 **45/45**，守卫双向咬人证明）；包体压缩态 +289,849B。前情：s65（探针转正 e2e 35/35+双竞态清账+goose 情报）；s64（报告 v2/UI 五缺陷/PG 取证）前情：s65（探针转正 e2e 35/35+双竞态清账+goose 情报）；s64（报告 v2/UI 五缺陷/PG 取证）报告 v2 探针转正（四 tmp 探针→tools/e2e/report-probe-{static,sandbox}.js+编排，挂 e2e 第 12 节，**e2e 升至 35/35**，5922d1d）；「浏览全部上限」零 diff 裁决（P29 起有 30/页分页，前提失实，STATE 勘误引以为戒）；EPIPE 竞态修复（第 11 节输出落地再 grep，a3adb0d，3×35/35）；沙箱 rmSync 间歇 EPERM 取证+修复（双因子：goose 孙进程句柄窗 0.1-0.3s + node v24 上游 Sleep 单位 bug 废掉 maxRetries；轮询重删 200ms/8s 四调用点，6ec8c29，5×29/29+2×35/35+等价复现 10/10）；goose 上游情报落档（research/04：不追 v1.49.0 等 v1.50 或退 v1.48.0，三红线源码级未动，f203083）+ 升级预案重建（abfbe3a）；矩阵 13 全归档态 GUI 全过（API 批量归档 175→复核→全量还原）。前情：s64（报告 v2/UI 五缺陷/PG 取证 BACKUP 挂 P32）
...
| s65-旧 | 报告探针转正ADR-0011(21068fa)→pm 裁决(zonky 17.11.0 实测 9.0s initdb/npm 全 beta 反证)→实施 d252478(21 条全过,pg-init+pg 两进程/端口动态拾取/零消费者零依赖)→qa 通过(决定性实验:占端口 pg 封顶停手其余 7 进程全绿);顺带修复停止脚本死词 shutdown→down(历史既坏);update-runner.js 同族死词修复中 | s66 |
...
- **PG 阶段一遗留（s66）**：①~~update-runner.js:151 死词~~ → s66 已修（3da0502，探针实证停栈 4.7s 真实生效；死词家族 shutdown→down 全清）；②pg readiness 探活 stale pid 假阳性（pg_ctl status 信 postmaster.pid；本机实测全权令牌下 pg 拒启restarts 耗尽后面板仍 Ready——阶段二引入消费者时探活加 TCP 复核或改由消费方自证）；③本机 dev 栈拉 pg 需降权令牌包装（runas /trustlevel:0x20000，内建 Administrator 默认提权+postgres 拒提权——目标机标准用户无此问题，纯本机环境事实）；②checksums.txt 与 fetch.sh regen 口径分叉（regen 丢 downloads/* 行——统一或声明手工维护面）；③FORGE_ROOT 含空格时 pc yaml 命令行未加引号（既有家族，真机护航观察）；④pc restarts 计数按会话累计不重置（阶段二随 max_restarts 翻转复核）；⑤dev/沙盒 data/pc.port 同值 8099 可跨杀（既有家族）；⑥停止脚本 down 后可加"确已停"校验；⑦~~阶段二动工前置：pg_dump 来源~~ → s66 已解（research/10：EDB zip 17.11-1=zonky 正源哈希实证，树内增量仅 pg_dump.exe 0.59MB，dump 实测通过；解压树在 downloads/edb-pg-17.11-1/）
...
- **观察项（s66）**：pg.log 探针噪音约 4.3MB/天（有独立 log_location 不进 pc.log/报告，256KB 预算无恙；pg.log 本身无轮换——真机 pg.log>50MB 或需排障时处置，候选 period 5s→15s）；data/sqlite 空目录理论竞态（先于本切片，观察）；runas wrapper pause 期间新实例静默不执行（自动化重拉前清旧 wrapper）
