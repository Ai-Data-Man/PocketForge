...
- 更新：2026-08-29 s51：用户小白可用性三主线落地并 GUI VERIFIED-RUN——管理面板六区块→六标签页（懒加载+重开重拉）/ 记忆清单搜索框+50条上限+显示更多 / 数据表人话化（/api/db/_schema 端点+点表名展开列清单：人话类型+真实样例值+pk「唯一」，表名列名保留原文护 @ 引用链路）；pf-qa 审查 4 项修复（懒加载缓存致面板不刷新回归 memAll=null TypeError fuzz 白名单钉子 err 回显截断）；e2e 26/26 + fuzz 23/23。业务含义 AI 翻译永久裁掉（错译比不译危险，别名表待真机固定业务库再议）。上一轮 s47→s50 系列：数据引用统一+DB可见性全链路 / 性能安全审计 12 修复 / P31-④收窄裁决 / FIND-0 归因收口 / v0.9.7 打包（护航候选）。部署缺口：全新包无 API key 首任务 401（向导首屏应含配Key步）
