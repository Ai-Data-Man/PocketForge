...
- **v0.9.9 发布候选（2026-09-05 就绪，未发布）**：dist/PocketForge-20260905-v0.9.9.zip（325,752,948B，sha256 7f5c6255…40d2c27，含欢迎页📮条目终版）= v0.9.8 后 66 提交全量（停止脚本真修复/报告 v2/UI 五修复/PG 阶段一+二/每日备份链）；沙盒全新冷启冒烟全过（含 fc948e9 守卫真实事故路径直证、重启幂等、报告零 PG 字节、停止无残留）；发布说明草稿 tmp/v0.9.9-release-notes-draft.md；**GitHub 发布待用户拍板**（需 push+GH_TOKEN），直接拷 dist zip 侧载亦可
...
- **PG 阶段一遗留（s66）**：①~~update-runner.js:151 死词~~ → s66 已修（3da0502）；②~~pg readiness stale pid 假阳性~~ → s66 阶段二已修（探活改 TCP，bin/pg-probe.js）；③~~本机拉栈需降权令牌~~ → 已操作化（e2e.sh 重拉走 runas 包装 8054c40；目标机标准用户无此问题，纯本机环境事实）；②checksums.txt 与 fetch.sh regen 口径分叉（regen 丢 downloads/* 行——统一或声明手工维护面）；③FORGE_ROOT 含空格时 pc yaml 命令行未加引号（既有家族，真机护航观察）；④pc restarts 计数按会话累计不重置（阶段二随 max_restarts 翻转复核）；⑤dev/沙盒 data/pc.port 同值 8099 可跨杀（既有家族）；⑥停止脚本 down 后可加"确已停"校验；⑥~~停止脚本 down 后"确已停"校验~~ → s67 已加（8054c40 轮询 pc API+人话警告；顺带根治 UTF-8 rem 在 GBK 代码页吃下一行的隐蔽缺陷——cmd 文件 ASCII 化纪律升级为硬规则）；⑦~~阶段二动工前置：pg_dump 来源~~ → s66 已解（research/10：EDB zip 17.11-1=zonky 正源哈希实证，树内增量仅 pg_dump.exe 0.59MB，dump 实测通过；解压树在 downloads/edb-pg-17.11-1/）
...
- **观察项（s66）**：pg.log 探针噪音约 4.3MB/天（独立 log_location 不进 pc.log/报告；pg.log 无轮换——真机 >50MB 或需排障时处置）；data/sqlite 空目录理论竞态（观察）；~~runas wrapper 静默不执行~~ → s67 已操作化（重拉前清旧 wrapper，8054c40/e2e.sh）；~~checksums 口径分叉~~ → s67 已统一（机器单一真相源 regen 幂等，8054c40）
