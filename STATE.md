...
- 更新：2026-09-06 s69（收尾）：**用户四主线全部落地**（裁决 2 份+研究底稿 2 份+代码 15 commits，e2e-chat **45/45** + fuzz **83/83** 全绿）。①**UI 残次猎捕**（qa 静态 26 条 P1×1/P2×12/P3×13 → 两批修复 24 条+主控 IAB 实证：mpick 勾选框被 `.frow input{width:100%}` 渗漏拉宽 440px（用户点名缺陷根因链）、`**加粗**` 渲染全废 P1、模型下拉冒泡复开、四处 esc 漏网、模态 ✕+Esc、流式粘性滚底、版本时间线样式、busy 护栏、夜间对比度等；P2-10/11 键盘读屏、P3-4 字号令牌化、P3-13 preview 主题化留账）。②**市场生态开放化**（裁决书 2026-09-05-marketplace-ecosystem：技能来源记录 origin.json market/self/local+同名冲突保护+原子安装（qa 两轮返工：徽标 XSS、覆盖失败保旧版）；源配置化 data/config/skill-sources.json+subdir 支持+**第二源 JimLiu/baoyu-skills 过闸激活**（MIT/双语/注入扫描零命中，research/13）；MCP 目录配置化 mcp-catalog.json+孤儿可卸载；分页挂起触发器制；PG 入裁决否决）。③**goose 透明度与防护**（research/12 源码级取证炸出三实锤：桥钉 GOOSE_MODE:'auto' 使 permission.yaml 从未生效、五 inspector 全不设防、dev AGENTS.md 进生产上下文；裁决书 2026-09-05-goose-guardrails 四切片全落：**默认模式回落 smart_approve+60s 超时改拒绝+never_allow 直通 Deny**（审批链激活，逃生口一键 auto）、四平台扩展收窄（58→47 把工具）、CONTEXT_FILE_NAMES 堵 AGENTS.md、定时配方 max_turns:10+手册三节；T3 挂起有据）。④**收尾返工闭环**（qa 三轮审查：切片A+UI批 2P2、最后四批 2P2——目录配置化三消费方半途+cfg-key dirty 跨档案 Key 覆盖，均修+钉子断言；mcpEnabled 显示链恒真顺修）。前情：s67（9router 结案+dev 栈宕机复盘+v0.9.9 三重验证）；s66（PG 阶段一+二）
- 阶段：**P31 内测护航进行中**（①本机部分 ✅（v0.9.9 候选包就绪未发布，GitHub 发布待用户拍板；**注意：s69 大量切片已入 dev 树，v0.9.10 内容已成形**），真机 POC 待用户；②收窄完成 ✅；③已就位；④收窄完成 ✅）
...
- **P32 候选（对标输入，research/11）**：①数据主权用户话术（Manus 2026-08 数据删除事件=本地便携形态的实证论据；载体注意：项目须知.md 从未落库【s44 声称已建，勘误见周期表】，届时需先建该文件或改挂使用说明）；②Plan Mode 三要素（随时/蓝图可编辑/确认执行）作任务选择层解冻的设计输入
