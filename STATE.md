...
- **P31 内测护航（下一周期）**：①真机 POC 打包与护航（新包 → C:\PocketForge-Test 全流程启动 → 首任务冒烟）；②首个任务向导（妻子打开后可选任务类型：抄数据/做报表/查记忆，预填流程）；③匿名本地使用统计（每天一次记录活跃会话数/报错计数/卡点关键词 → 驱动 P32+ 决策）；④多线路韧性（provider 故障自动降级 UI，glm-5.2 401/429 教训落地为产品特性）
