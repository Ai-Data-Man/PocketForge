...
- 更新：2026-09-06 s69（收尾，两波）：**用户四主线+升级批全部落地**（裁决 3 份[1 份补档]+研究底稿 2 份+代码 20 commits，e2e-chat **46/46**（+ui-logic 探针节）+ fuzz **102/102** 全绿）。①**UI 残次猎捕**：qa 静态 26 条全清（三批：d5a7d17/6fdecd8/4467ff9——mpick 440px 渗漏根因[用户点名]、`**加粗**` 全废 P1、模态 ✕+Esc、粘性滚底、键盘可达 ↑↓ 高亮/IME 守卫[f6ec76a]、读屏语义最小集、字号 21 处 px→rem 随根缩放、preview.html 昼夜自适应、夜间对比度等）；主控 IAB 实证多轮。②**市场生态开放化**（裁决书 marketplace-ecosystem）：origin.json 来源记录+冲突保护+原子安装（qa 返工：徽标 XSS/覆盖失败保旧版）；skill-sources.json 多源+subdir+**第二源 baoyu-skills 进交付默认**（dft() 双源+_schema:1→2 迁移追加，f6ec76a）；mcp-catalog.json 配置化+三消费方补全+孤儿可卸载；分页挂触发器。③**goose 防护**（research/12+裁决书 goose-guardrails，S-A~S-D 全落）：**聊天默认模式回落 smart_approve（permission.yaml 自此真实生效）+60s 超时自动拒绝+never_allow user:段直通 Deny**；四平台扩展收窄（工具 58→47）；CONTEXT_FILE_NAMES 堵 AGENTS.md 泄漏；定时配方 max_turns:10；mcpEnabled 显示链顺修。④**离线升级通道收口**（裁决书 offline-upgrade-sha[s67 正文缺失 s69 补档 b54cf15]）：桥端 .sha256 自算自验+魔数/字节/哈希三重校验+.part 原子落位（1c4a1e0，fuzz +17）+**UI 两步向导**（c8a1103，主控 IAB 实证）+**升级 config 保全**（64feedb PROTECTED config.yaml/custom_providers/memory，s69 遗留①修复）；qa 第三轮返工（IME 劫持/第二源进默认/路径泄漏人话化/.part 独占）。发布说明草稿+真机护航须知已入库（24466d9）。前情：s67（9router 结案+v0.9.9 三重验证）；s66（PG 阶段一+二）
