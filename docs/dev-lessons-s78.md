...
- **#30（s99→s100，2026-09-22）：无研究不设计**——s99 能力编辑器把 s98 家族别名机制直接翻译成「给它配一个深档：深档用哪个模型」，用户一句话推翻（"正常人谁会把思考深度设置等同于换模型"）。根因=设计输入只有两张截图+s98 单线实证，没解剖 ZCode 开源代码（本机 app.asar 解剖后：正确抽象=同模型档位集 optionSpecs.reasoningLevel.values + 翻译层 CEL map，全库 0 处换模型形态）。伴生三缺陷：inline 展开非弹窗/池尾加模型/池无搜索。修复裁决=docs/verdicts/2026-09-22-capability-config-v2-zcode.md；纪律固化=AGENTS.md §9（研究出处节+负验证+一句话推翻=过门条件+机制默认不进配置面）。
