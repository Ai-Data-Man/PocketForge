...
## 追加(0825):提示词自动注入工作区路径
- 闭环 journal 上条遗留:submit() 现在自动在提示词尾部追加一行系统备注(当前 ws 目录+版本管理提醒),仅当用户文本未含 data/artifacts/ 路径时;聊天气泡仍显示用户原文。
- 契约实测:同一载荷直驱 goose → agent 精准写入 `data/artifacts/<ws>/probe-ws.txt`(此前无提示会写到 forge 根)。
- IAB 教训补充:某些页面状态下 Playwright press('Enter') 与 cua keypress 均被吞而 type() 字符事件正常——验证提交类交互优先点「发送」按钮或换页面实例,不要把自动化层怪癖误判为产品 bug。
