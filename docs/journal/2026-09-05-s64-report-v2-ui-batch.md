...
- pf-pm 裁决书（docs/verdicts/2026-09-05-report-v2.md）：真问题="丈夫3分钟形成首个假设"，初诊=规则查表（确定性/离线/只说不做），LLM 诊断裁掉（agent 挂掉恰是主场景）；隐私逐条裁定（定时任务只出两计数、技能只出数量、MCP 只出 name+enabled 结构级排除）；体积硬顶 256KB。
