...
## QA 复审 s83d（a7890ad+bea6ee7 双通过）+T-1 修（cacdb41）
- 单飞修复四重点全证（key 构造 NUL 碰撞不可达/p 永不 reject 无污染/finally 必经无泄漏/**进程级铁证：50 并发 faucet.exe 峰值=3**）；tblMiss 双 flavor 活体全通；fuzz 187/187 复现。
- T-1 探针假红一行修（快照移预清后，植残留复现转绿双跑验证）；💭 留档五条（F-1 空清单遮蔽注行/F-4 hang 时延翻倍 ~10.3s 未观测等）。
...
[产出循环 20:4x] 套件尾扫窗生效验证：e2e 58/58 跑毕窗口=0（自洁落地，412f583）；FINDING-2 从「人工跟扫」升级为「套件自洁」销案

[产出循环 20:3x] appcap 七跑 13/13（真模型能力基线恒定）+healthz ok

[产出循环 20:3x] 组合态 GUI 走查：当前栈有 key 有模型（key-guide 不显=正确互斥语义）；made tab 空态文案正确零 pageerror。UX 审计「无 key/stale 组合态」在 dev 栈不可布景（有真配置），其活体复现留沙盒专项（低增量，挂观）。

[产出循环 20:4x] 性能基线快照：/api/assets 冷 275ms（18 条）/热 403ms（含 fs 扫描波动）/db/overview 199ms——护航规模（<100 条）远在 R2 档内；goose 进程 38（1 acp 主+闲置会话扩展树，桥重启可整树回收——既有裁决面）。

[产出循环 20:3x] 会话库盘点：743 会话/4667 消息/289 工作区（dev 树两个月测试累积；桥自修剪护栏=空壳 1 天自清+归档内可删——不动用户数据纪律，健康）

[产出循环 20:3x] 教训册引用交叉校验：tmp 引用全在案、五关键文件全在——记忆层完整性过

[产出循环 20:4x] 假 9router 取证资产复活（:20129）——provider 复测工具箱恢复完整（栈注记同步：fake9r=alive）

[产出循环 20:4x] welcome 三新块在案复核过

[产出循环 20:40] 周期全树核验第二轮：e2e 58/58+fuzz 187/187+窗口 0（基线三连恒定）

[产出循环 20:5x] **升级路径差量预演**（两真实包对账）：25,282 相同/10 变 8 增 1 删；**exe 变更恰 1=goose.exe（v1.50）→ 信誉尖峰版确认**（发布说明 EDR 段数字修正为实测）；顺带抓修 permission.yaml.lock 随包发布卫生缺陷（1871156——package.sh 排除补一行）。

[产出循环 20:56] 三件套核验（e2e 58/58+fuzz 187/187+appcap 13/13）三连全绿+窗口 0——基线在打包卫生修复后恒定

[产出循环 21:0x] 发布就绪盘点：草稿/演练包/SHA256-EXE 全在；**演练包已过期**（含 permission.yaml.lock+不含 1871156 后的 package.sh+不含 F-1/后续修复）——正式发布时须重打包（release.sh 首跑会自然重打，无需额外动作；此条为预检记录）。发布三条件齐：①用户拍板时机 ②tag+打包（自动） ③GH_TOKEN。

[产出循环 21:08] 第 18 轮：e2e 首跑 57/58（§18c ws-rollback 单红）→全套复跑 58/58+探针 standalone 三连 31/31 全绿——判套件内偶发（沙盒批载竞态家族既有观察项第 N 数据点，与 tree 零因果：本轮窗口零代码变更）；fuzz 187/187 同窗绿。处置：记录不追（该家族已归档「净机绿即过」）。

[产出循环 21:1x] 资源基线：RAM 5.1/16GB 空闲、盘 37.9GB、桥 RSS 368MB、goose 65 进程（闲置会话扩展树——自 s75 已知面，桥重启整树回收；node 132=各探针残留+树内工具）。资源充裕可支撑长窗循环。

[产出循环 21:17] 第 20 轮：闲置 goose 进程树回收演练——65→2（桥重启整树回收机制 VERIFIED-RUN 第 N 次，RL 释放）；回收后三件套核验 58/58+187/187 全绿。

[产出循环 21:18] 第 21 轮：goose 上游仍 v1.50.0 稳定；栈绿；goose 进程 10（本轮套件的正常增量——下轮回收窗再清）。

[产出循环 21:30] 第 22 轮：标准循环单元三件套全绿（58/58+187/187+appcap 13/13）+回收复式 goose→2。基线五连恒定。

[产出循环 21:4x] 第 23 轮：循环单元脚本化（tmp/cycle-unit.sh）首跑全绿（58/58+187/187+13/13+goose=2；healthz 空系脚本 node fetch 在 bash 管道内同步退出怪癖，独立复查=ok）——后续轮次一键执行。

[产出循环 21:5x] 第 24 轮：§18c 套件内红第 2 数据点（6 轮 2 红，standalone 恒绿）——超「偶发」档，QA 专项已派（红签名取证）；循环单元补 60s 静息门（对齐 e2e↔fuzz 稳定间隔纪律）。

## QA 专项 s83e：§18c 套件内红（12a87d4）
- 16 样本矩阵零复现；红签名形态学（PASS=26 FAIL=1 算术定位+历史残骸 0 字节 llm_request）=宿主级瞬态饥饿/幼龄进程死亡族，产品侧零缺陷证据零修复（有意不重试不放宽——掩蔽回归且对进程死亡无效）。
- 基建硬化：ck 时间戳/桥 exit 钩子/绿清红留沙盒/红时归档回显——再红即定案（日志可直接钉死中止点）；降级判据=静息门后 30 轮零复发销观察。
- QA 自省三条入册（含清理越界误删两件历史 tmp——教训：rm 边界）。

[产出循环 23:20] 第 25 轮（静息门版首跑）：三件套全绿 58/58+187/187+13/13+goose=2——§18c 观察窗第 1/30 轮零复发。

[产出循环 23:34] 第 26 轮全绿（§18c 观察窗 2/30 零复发）

[产出循环 23:46] 第 27 轮全绿（§18c 观察窗 3/30 零复发）

[产出循环 23:58] 第 28 轮全绿（§18c 观察窗 4/30 零复发）

[产出循环 00:09] 第 29 轮全绿（§18c 观察窗 5/30 零复发；跨午夜连续性正常）

[产出循环 00:23] 第 30 轮全绿（§18c 观察窗 6/30 零复发）

[产出循环 00:36] 第 31 轮全绿（§18c 观察窗 7/30 零复发）

[产出循环 00:50] 第 32 轮全绿（§18c 观察窗 8/30 零复发）

[产出循环 01:00] 第 33 轮全绿（§18c 观察窗 9/30 零复发）

[产出循环 01:13] 第 34 轮全绿（§18c 观察窗 10/30 零复发）

[产出循环 01:25] 第 35 轮全绿（§18c 观察窗 11/30 零复发）

[产出循环 01:36] 第 36 轮全绿（§18c 观察窗 12/30 零复发）

[产出循环 01:49] 第 37 轮全绿（§18c 观察窗 13/30 零复发）

[产出循环 01:58] 第 38 轮全绿（§18c 观察窗 14/30 零复发）

[产出循环 02:09] 第 39 轮全绿（§18c 观察窗 15/30 零复发）
...
[产出循环 02:50] 第 42 轮全绿（§18c 观察窗 16/30 零复发）

[产出循环 03:01] 第 43 轮全绿（§18c 观察窗 17/30 零复发）

[产出循环 03:10] 第 44 轮全绿（§18c 观察窗 18/30 零复发）

[产出循环 03:21] 第 45 轮全绿（§18c 观察窗 19/30 零复发）

[产出循环 03:32] 第 46 轮全绿（§18c 观察窗 20/30 零复发）

[产出循环 03:41] 第 47 轮全绿（§18c 观察窗 21/30 零复发）

[产出循环 03:50] 第 48 轮全绿（§18c 观察窗 22/30 零复发）

[产出循环 04:00] 第 49 轮全绿（§18c 观察窗 23/30 零复发）

[产出循环 04:10] 第 50 轮全绿（§18c 观察窗 24/30 零复发）——距窗口终点约 50 分钟

[产出循环 04:21] 第 51 轮全绿（§18c 观察窗 25/30 零复发）

[产出循环 04:32] 第 52 轮全绿（§18c 观察窗 26/30 零复发）——窗口终点前最后一轮完整循环

[产出循环 04:44] 第 53 轮全绿（§18c 观察窗 27/30 零复发）——窗口终点前最后完整记录

## 会话收工总账（窗口 09-12 14:47 → 09-14 05:00 达成）
- **提交 186+**（38b06be→c5784c7 及后续）；**产出循环 53 轮**（无 sleep 值守，每轮三件套核验/环境 tick/深挖项，全部 journal 留痕）。
- 终态基线恒定：e2e-chat 58/58 + fuzz 187/187 + appcap 13/13；§18c 静息门后 27 轮零复发。
- 全部主线/调研七题/应用管理特性/QA 六轮复审/四大根因战役/goose v1.50/发布备料/两册沉淀（协调手册+24 条教训）/s84 端口动态化——详见 docs/s78-session-deliverables-index.md。
...
## 🛡️ 云主机爆破处置完成（2026-09-14 15:0x）
- **三地 IP 定位（证据驱动）**：用 TS-LSM 全量日志（05-20→09-14，1845 条 / 含来源成功会话 545 条）做行为画像 + 三源 geo 核验 → 武汉·公司 `27.17.34.18`（工作日 9-11 时峰、44 天固定）；武汉·出租屋 `171.83.x` 5G 池（全周、晚/深夜峰、141 会话最高频）；襄阳·家 `27.27.16x`（周末型）。与用户口述武汉两个+襄阳一个吻合。
- **根因**：先前只收敛了端口级放行规则来源，但系统存在多条 Port=Any + Remote=Any 放行规则（Windows Search / 你的帐户 / 开始 / 桌面应用 Web 查看器 / 安全中心 / sing-tun 等）绕过端口收敛 → 爆破不停。
- **修法**：不改系统规则，改用**显式 BLOCK（阻断优先）**——`PF-SEC-BLOCK-IN-TCP`（135,139,445,3389,5985,47001）+ `PF-SEC-BLOCK-IN-UDP`（137,138,3389），RemoteAddress=7 段全网段减三地放行空间（私网故意保留）。同时对 3389/5985 既有放行规则按端口匹配收敛至三地空间。
- **验证**：下发前 4625≈14/分 + 4 条攻击者 TCP 连接 → 下发后攻击者连接即刻清除、用户 RDP 会话（27.17.34.18:41087）不断线；5×45s 采样全部 `4625_3min=0 / 131_3min=0`；本机回环业务端口（8790/8099/8092/5432/4222）不受影响。
- 详见 `docs/verdicts/2026-09-14-rdp-bruteforce-disposition.md`（含回滚命令、云安全组建议、账户锁定策略建议、脚本清单）。
