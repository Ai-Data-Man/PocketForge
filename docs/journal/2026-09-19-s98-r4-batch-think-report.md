...
## 后段：用户四批评返工波（同日续，批评①思考假把式②三分法无语义③数据面板忽略④技能100+；③⑤轮用户改向「机制三分是对的，做成一个本事很奇怪」）

- **取证与裁决**：research/37（思考力度真杠杆：9router×glm 参数通道 5 样本×5 臂死透；glm-5.3 完整版=真深档 4x 思考量/3/3 正确率/2.8x 成本；goose 双条目+会话内切模型 wire 双落验证；诚实上限两档）+ r5 裁决（capability-semantics：三分骨架零动治概念层——自带的/工具/手艺三词三母句+归置审计零挪家病灶=近义对描述不划界+手艺100+预测性立项：停启=SKILL.md↔.off 双盲改名、成本在常驻上下文每请求在场源码级证实）。
- **工程五批**：db-scale 5a26bca（行数预取风暴删+护栏 2000+搜索全覆盖；faucet schema 全量 2050 表实证）；think-grad 58228dc（四态 select：原生五档/**快/深切 -flash 兄弟模型（复用 switch_model，真栈活体切深=wire 模型帧翻）**/诚实禁用/无键隐藏，桥零改动）；R5-S1 271865e（词汇换血+三母句+slash/台账/技术区）；R5-S2 ca8e0d8（LABELS/MCP_CATALOG/出厂 SKILL.md 划界句+物化 cmp）；R5-S3 f0a83e0（停启 op+active 字段+chips+批量停启+slash 溢出+**update-runner twin-guard：升级 .off 孪生保鲜写入+.agents/skills 差量删除全跳——两个超裁决预期的真缺陷（升级复活停用态/差量误删用户手艺）被 V2 活体逮住并当日修**；V1/V4 活体=停启同进程下回合即生效）。
- **QA 复审（tmp/qa-s98rework-review.md）：返工两 P2+P3+P4×2 → 定向修 4cf2b60 全红绿闭合**：P2-1 桥侧旧词漏网（faucet-db desc「本事」活体证伪探针扫描面盲区→清扫 8 处+探针扩桥面+活体断言）；P2-2 停启 rename 失败反射绝对路径（PowerShell 锁活体复现→humanToggleErr 人话门）；P3 twin-guard 停用态内容陈旧（.off 孪生异容→新内容写进 .off，启用即得新版）；P4-a restarted 切模型补 configOptions；P4-b skillstore op 严格枚举门。变异证红×2（妻面注入旧词/剥孪生守卫）。**终态基线 e2e 62+fuzz 225+ui-logic 56+ia-logic 34**。
- **手册词汇同步**（515f87b）：使用说明全量换血+手艺停启用法+思考力度快/深两档说明+数据面板多表可搜。
- **iat18 出包**（6610ad34…，323,653,021B）；从零复验在飞（s98d：新词汇全链/快深真切换/停启/多表面板）。
