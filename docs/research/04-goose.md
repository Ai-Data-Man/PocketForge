...
4. ~~9router 大工具集缺陷~~ → **已结案（s67，2026-09-05 复测不可复现）**：deepseek-v4-flash 上 N=1..50（含事发区间 40+）28/28 采样全部正常 tool_calls、零 502（阈值曲线 VERIFIED-RUN，探针 tmp/s67-router-retest/）；s19 当夜的归因在 journal s20/s22 已撤回（真因=IE 系统代理劫持 + glm-5.2 线路账号锁定），本条此前漏同步。9router 现为 0.5.65（09-03 更新）。风险降级为观察：glm-5.2 未复测（用前单发 N=40 探针即可）；环境铁律=手工探针必须 NO_PROXY=127.0.0.1,localhost 或直连 TCP（生产链已注入）。
