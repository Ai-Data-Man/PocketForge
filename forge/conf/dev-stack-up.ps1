...
& (Join-Path $ForgeRoot 'bin\pc\process-compose.exe') up -f (Join-Path $ForgeRoot 'conf\process-compose.yaml') -f (Join-Path $ForgeRoot 'conf\ports.env.yaml') -f $out -p $PC_PORT -t=false
